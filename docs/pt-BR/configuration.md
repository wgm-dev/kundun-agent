...
aplicadas. O comando `kundun cleanup` é sempre explícito; veja [Limpeza](cleanup.md).
...
> [Motor de memória](memory-engine.md) e [Limpeza](cleanup.md).
...
- [Visão geral / Documentação](../README.md) — índice da documentação.
- [Primeiros passos](getting-started.md) — instalação e primeiro `kundun init`.
- [Scanner e indexação](scanner-indexing.md) — como `include`/`exclude`, arquivos
  sensíveis e linguagens afetam a varredura.
- [Limpeza](cleanup.md) — como as políticas de `cleanup` são aplicadas.
