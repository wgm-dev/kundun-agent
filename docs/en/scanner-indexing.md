...
[search](search.md), [symbol lookup](search.md), and the [summary](cli-reference.md).
...
1. Walk the project respecting `include` / `exclude` globs and the root
   `.gitignore` (see [Configuration](configuration.md)).
...
4. Any tracked file that is no longer present on disk is marked `is_deleted=1`
   (**removed**). Its row stays so that [cleanup](cleanup.md) can retire it on the
...
This is a core part of Kundun-Agent's [security](security.md) posture: the index can
...
helps the [summary](cli-reference.md) surface what matters and helps
[cleanup](cleanup.md) reason about retention. Scoring is heuristic, based on the kind
of file:
...
sync as it writes chunks, so full-text [search](search.md) reflects the latest
...
- [Search](search.md) — querying the chunks and symbols this stage produces
- [Security](security.md) — the full local-first and sensitive-file guarantees
