...
Like every command, `cleanup` accepts the [global options](cli-reference.md)
...
`enableAutoCleanup` flag — see the [configuration](configuration.md) page. You can
...
or more. See the [memory engine](memory-engine.md) page for how importance is set
...
[`summary`](cli-reference.md) overview to see when the last cleanup ran.

## See also

- [Documentation hub](../README.md)
- [Memory engine](memory-engine.md) — importance scoring and why
  high-importance memories survive cleanup.
- [Configuration](configuration.md) — the full `cleanup` retention keys and
  `enableAutoCleanup`.
