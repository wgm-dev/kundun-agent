...
These checks live in the scanner; see [Scanner & Indexing](scanner-indexing.md) for
...
the file can be tracked for deletion by [cleanup](cleanup.md) — but the file's
...
- [Scanner & Indexing](scanner-indexing.md) — where the containment and
  sensitive-file checks run
- [Configuration](configuration.md) — `include` / `exclude`, `maxFileSizeKb`, and
  cleanup retention
