...
- **[Configuration](configuration.md)** — every key in `kundun.config.json`,
  ...
  retention.
- **[CLI reference](cli-reference.md)** — the complete command and flag list.
- **[Scanner & indexing](scanner-indexing.md)** — how files are walked, chunked,
  and scored, and how sensitive files are handled.
- **[Search](search.md)** — FTS5 vs. the LIKE fallback and how ranking works.
- **[Memory engine](memory-engine.md)** and **[Task engine](task-engine.md)** —
  persisting knowledge and tracking work.
- **[Cleanup](cleanup.md)** — retention rules and what `--dry-run` does.
