---
title: CLI Reference
description: The authoritative command list for the kundun CLI — every command and subcommand, with exact flags, arguments, example invocations, and global options.
---
...
See [`scanner-indexing.md`](/en/scanner-indexing/) for how files are selected,
skipped, and chunked.
...
See [`search.md`](/en/search/) for ranking and fallback details.
...
See [`memory-engine.md`](/en/memory-engine/) for retrieval, promotion, and
archival behavior.
...
See [`task-engine.md`](/en/task-engine/) for the full task lifecycle.
...
See [`cleanup.md`](/en/cleanup/) for retention defaults and the transaction model.
