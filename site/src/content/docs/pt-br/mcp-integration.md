---
title: Integração MCP (Claude Code, Codex, Cursor…)
description: O Kundun-Agent inclui um servidor MCP para que agentes de programação compatíveis com MCP possam chamar seus recursos como ferramentas (tools).
---
...
[especificação completa](https://github.com/wgm-dev/kundun-agent/blob/main/README.md) §18 para os formatos de entrada):
...
- **Sessões, saúde, métricas e eventos estão ativos.** O servidor registra uma
  ...
  rode também `kundun daemon` (veja o guia do [Dashboard web](/pt-br/dashboard/)).
